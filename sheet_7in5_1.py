...
class TearOffCalendarSheet(TearOffCalendarBaseSheet):
    ...
    def draw(self, cal_data = None):
        ...
        self.draw_decor_corners(self.pages[self.j], 32)
        ...
        self.draw_day(self.draw[self.i], self.cal_data['day'], 200, 'AbrilFatface-Regular.ttf', 200)
        ...
        if self.cal_data['forecast']:
            self.draw_forecast(self.pages[self.BLACK], self.draw[self.BLACK], self.cal_data['forecast'], 575, 61, (32, 69, 32), 'Cuprum-Regular.ttf', 29, (5, 3, 1))
        else:
